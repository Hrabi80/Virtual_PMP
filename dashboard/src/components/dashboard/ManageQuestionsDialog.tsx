...
import { ManageBonusDialog } from "./ManageBonusDialog"
...
export const ManageQuestionsDialog = ({ open, onOpenChange, category }: ManageQuestionsDialogProps) => {
  ...
  const getTotalBonusPoints = (question: Question) => {
    ...
    return question.bonuses.reduce((total, bonus) => total + bonus.bonusPoints, 0)
  }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Add, edit, or delete questions within this category. Configure bonus points for question sequences.
            </DialogDescription>
          </DialogHeader>
              ...
              {!category?.questions || category.questions.length === 0 ? (
                ...
                <div className="space-y-4">
                  {category.questions.map((question) => (
                    <div key={question.id} className="border rounded-lg bg-white">
                      <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value={question.id} className="border-0">
                              ...
                              <div className="flex items-start space-x-4">
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center space-x-2 mt-2">
                                    ...
                                    {question.bonuses && question.bonuses.length > 0 && (
                                      <Badge variant="secondary" className="bg-green-100 text-green-800">
                                        +{getTotalBonusPoints(question)} bonus pts
                                      </Badge>
                                    )}
                                  </div>
                                </div>
                          ...
                          <AccordionContent className="px-6 pb-4">
                            <div className="space-y-4">
                              <div>
                                <h5 className="text-sm font-medium text-gray-900 mb-2">Virtual Patient Response:</h5>
                              ...
                              {question.medicalPictureUrl && (
                                <div>
                                  <h5 className="text-sm font-medium text-gray-900 mb-2">Medical Picture:</h5>
                                  <p className="text-sm text-blue-600">{question.medicalPictureUrl}</p>
                                </div>
                              )}

                              {question.bonuses && question.bonuses.length > 0 && (
                                <div>
                                  <h5 className="text-sm font-medium text-gray-900 mb-2">Bonus Links ({question.bonuses.length}):</h5>
                                  <div className="space-y-2">
                                    {question.bonuses.map((bonus) => (
                                      <div key={bonus.id} className="flex items-center justify-between p-2 bg-green-50 rounded-md">
                                        <div className="flex items-center space-x-2">
                                          <Link className="h-4 w-4 text-green-600" />
                                          <span className="text-sm text-gray-900">
                                            Links to: "{bonus.linkedQuestionText || 'Unknown Question'}"
                                          </span>
                                        </div>
                              ...
                              <div className="flex items-center justify-between pt-2 border-t">
                                ...
                                <div className="flex items-center space-x-1">
                                  <span className="text-xs text-gray-500">Total Possible Points:</span>
                                  <Badge variant="outline">
                                    {question.score + getTotalBonusPoints(question)} pts
                                  </Badge>
                                </div>
                              </div>
                            </div>
